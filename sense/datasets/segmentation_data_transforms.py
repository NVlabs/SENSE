...
class RandomCrop(object):
    def __init__(self, size, downsampling_rate):
    ...
    def __call__(self, image, label, *args):
        ...
        if label is not None:
            ...
            label = label.resize((label.width // self.downsampling_rate,
                                  label.height // self.downsampling_rate),
                                  Image.NEAREST)
...
class ToNumpy(object):
    ...
    def __call__(self, image, label=None):
        image = np.array(image).astype(np.float32) * self.coef
        if label is None:
            return image
        else:
            return image, label
...
class RandomBrightnessImg(object):    
    def __init__(self, val_sigma):
        self.val_sigma = val_sigma

    def __call__(self, image, label=None):
        value = np.random.normal(0, self.val_sigma, 1)
        image = np.clip(0.0 + image + value[0], 0.0, 1.0)
        if label is None:
            return image
        else:
            return image, label
...
class RandomContrastImg(object):    
    ...
    def __call__(self, image, label=None):
        ...
        for c in range(0, 3):
            ...
            image[:, :, c] = np.clip(1.0*(image[:, :, c] - cur_mean)*value+cur_mean, 0.0, 1.0)
        if label is None:
            return image  
        else:
            return image, label
...
class RandomGaussianNoiseImg(object):
    ...
    def __call__(self, image, label=None):
        ...
        image = np.clip(0.0 + image + noise, 0.0, 1.0)
        if label is None:
            return image
        else:
            return image, label
