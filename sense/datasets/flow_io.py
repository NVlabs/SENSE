...
def flow_read_uv(filename, layernumber=0):
    """ Read optical flow from file, return UV array. 
    ...
    size = width*height
    assert width > 0 and height > 0 and size > 1 and size < 100000000, ' flow_read:: Wrong input size (width = {0}, height = {1}).'.format(width,height)
    uv = np.fromfile(f,dtype=np.float32,count=-1).reshape((height,width, 2))

    uv = np.swapaxes(np.swapaxes(uv, 1, 2), 0, 1)    # height*width*nchannels -> # nchannels*height*width

    # crop flow
    if layernumber >0:
        width = int(math.floor(width/(2**layernumber))*(2**layernumber))
        height = int(math.floor(height/(2**layernumber))*(2**layernumber))
        uv = uv[:,0:height,0:width]
